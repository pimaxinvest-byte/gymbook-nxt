    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in">
        ...
        <div className="flex items-start justify-between flex-wrap gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="grid grid-cols-4 gap-4 text-center">
              {[
                { label: 'Kcal', value: plan.targetCalories, color: '#3B82F6' },
                { label: 'Prot', value: `${plan.proteinG}g`, color: '#22C55E' },
                { label: 'Carbs', value: `${plan.carbsG}g`, color: '#EAB308' },
                { label: 'Grasa', value: `${plan.fatG}g`, color: '#F59E0B' },
              ].map((m) => (
                <div key={m.label}>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <GenerateWeeklyPlanBtn planId={plan.id} hasExisting={!!plan.weeklyPlan} />
        </div>
                ...
                <div className="p-1.5 space-y-1">
                  {(byDay[day] ?? []).map((meal: any) => (
                    ...
                    >
                      ...
                      <p className="text-[11px] text-text leading-tight">{meal.name}</p>
                      <p className="text-[9px] font-mono text-primary/70 mt-0.5">{meal.calories} kcal</p>
                    </div>
                  ))}
                </div>
              </div>
